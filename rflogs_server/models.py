...
class FileInfo(BaseModel):
    ...
    name: str = Field(
        ...
        json_schema_extra={"example": "log.html"}
    )
    path: str = Field(
        ...,
...
class TimingStats(BaseModel):
    ...
    std_deviation: float = Field(..., description="Standard deviation of execution time in seconds")
...
class ParsedRunStats(BaseModel):
    ...
    verdict: str = Field(..., description="Overall verdict of the run. Values: 'pass' or 'fail'.")
    ...
    failed_test_names: List[str] = Field(default_factory=list, description="Names of the failed tests.")
...
class RunInfo(BaseModel):
    ...
    public_access: bool = Field(..., description="Indicates if the run is publicly accessible.")
    files: List[FileInfo] = Field(..., description="List of files associated with the run.")
    created_at: datetime = Field(..., description="Timestamp when the run was created.")
    total_tests: Optional[int] = Field(None, description="Total number of tests executed.")
    ...
    skipped: Optional[int] = Field(None, description="Number of tests that were skipped.")
...
class ProjectRunsResponse(BaseModel):
    ...
    is_owner: bool = Field(..., description="Indicates if the authenticated user is the owner.")
    storage_used: int = Field(..., description="Total storage used by the project in bytes.")
    total_results: int = Field(..., description="Total number of runs matching the query.")
